...
function makeSummary() {
  document.getElementById('smr').onclick = function (ev) {
    deletePreviousInfo();
    
    // Create form element
    let form = document.createElement('form');
    
    // Create color picker for changing text color
    form.innerHTML += '<label for="colorPicker">Choose Text Color: </label>';
    form.innerHTML += '<input type="color" id="colorPicker" name="colorPicker" value="#000000">';
    
    // Create font selector for changing font
    form.innerHTML += '<label for="fontSelect">Choose Font: </label>';
    form.innerHTML += '<select id="fontSelect" name="fontSelect">' +
    ...
    // Add textarea for the summary
    form.innerHTML += '<textarea style="word-wrap:break-word;" class="form-control" id="summaryText" placeholder="Summary" rows="7"></textarea>';
    ...
    // Set up event listener for color and font changes
    document.getElementById('colorPicker').onchange = function () {
      document.getElementById('summaryText').style.color = this.value;
    };
    
    document.getElementById('fontSelect').onchange = function () {
      document.getElementById('summaryText').style.fontFamily = this.value;
    };

    putSummary();  // Call putSummary function if needed
  };
}
...
function putSummary() {
  document.querySelector('form').onsubmit = function (ev) {
    ...
    // Capture the color and font selected by the user
    ...
    // Get the summary content from the textarea
    let summaryContent = document.querySelector('.form-control').value;

    // Set the summary content and apply the selected styles
    let target = document.querySelector('#summary p');
    target.textContent = summaryContent;
    target.style.color = selectedColor;  // Apply the selected color
    target.style.fontFamily = selectedFont;  // Apply the selected font
    
    styleSummary();  // Additional styling if needed
  };
}
